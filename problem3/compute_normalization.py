"""
This scripts compute the normalization parameters (mean and standard deviation), per channel,
...
def test(traindir):
    """
     Test to see that data is actually normalised.
    """
    ...
    for i in range(len(train_dataset)):
        ...
        print('mean ', torch.mean(img[0, :, :]))
        print('std ', torch.std(img[0, :, :]))
