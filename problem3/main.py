...
def train(args, model, device, train_loader, optimizer, epoch, experiment):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        experiment.log_metric("accuracy",  100. * correct / total, step=batch_idx)
...
def test(args, model, device, test_loader, experiment):
    model.eval()
    test_loss = 0
    correct = 0
    with torch.no_grad():
...
def main():
    ...
    with experiment.test():
        test(args, model, device, test_loader, experiment)
